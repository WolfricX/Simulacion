...
export const LogoutService = async () => {
  try {
    ...
    return await genericRequest(base_url + '/users/logout', 'POST', { token })
  } catch (error) {
...
export const ValidateTokenService = async (token: string) => {
  try {
    return await genericRequest(`${base_url}/users/validate-token`, 'POST', { token });
  } catch (error) {
    await handleError(error as AxiosError, 'validate-token');
  }
};
